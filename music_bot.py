...
def start_background_tasks():
    ...
    try:
        ...
        # Запускаем задачу очистки кэша поиска каждые 15 минут
        asyncio.create_task(run_periodic_task("Очистка кэша поиска", cleanup_search_cache, 900))
        ...
        logging.info("🚀 Мониторинг производительности активирован")
...
# Оптимизация: пул соединений для HTTP запросов
import aiohttp
session_pool = None

# Оптимизация: настройки для быстрой работы
FAST_DOWNLOAD_OPTS = {
    'format': 'worstaudio/worst',  # Быстрее загружается
    'quiet': True,
    'no_warnings': True,
    'ignoreerrors': True,
    'extract_flat': True,
    'timeout': 15,  # Уменьшаем таймаут
    'retries': 1,   # Уменьшаем попытки
    'nocheckcertificate': True,  # Пропускаем проверку сертификатов
    'prefer_ffmpeg': False,  # Не используем FFmpeg для метаданных
}

# Оптимизация: настройки для качественной загрузки
QUALITY_DOWNLOAD_OPTS = {
    'format': 'bestaudio/best',
    'quiet': True,
    'no_warnings': True,
    'ignoreerrors': True,
    'extract_flat': False,
    'timeout': 120,  # 2 минуты для загрузки
    'retries': 2,
...
def cleanup_search_cache():
    """Очищает кэш поиска, удаляя устаревшие записи"""
    global search_cache
    try:
        current_time = time.time()
        keys_to_remove = []
        ...
        if keys_to_remove:
            logging.info(f"🧹 Кэш поиска очищен: удалено {len(keys_to_remove)} записей")
        
        # Если кэш все еще слишком большой, удаляем старые записи
        if len(search_cache) > SEARCH_CACHE_MAX_SIZE:
            items_to_remove = int(SEARCH_CACHE_MAX_SIZE * 0.3)
            keys_to_remove = list(search_cache.keys())[:items_to_remove]
            for key in keys_to_remove:
                del search_cache[key]
            logging.info(f"🧹 Кэш поиска дополнительно очищен: удалено {len(keys_to_remove)} записей")
    ...
    try:
        if query in search_cache:
            ...
            if time.time() - timestamp < SEARCH_CACHE_TTL:
                logging.info(f"💾 Используем кэшированные результаты поиска для '{query}'")
                return results
            else:
        ...
        logging.error(f"❌ Ошибка получения кэша поиска: {e}")
        return None

def cache_search_results(query: str, results):
    """Кэширует результаты поиска"""
    global search_cache
    try:
        # Очищаем кэш перед добавлением новых данных
        cleanup_search_cache()
        
        search_cache[query] = (time.time(), results)
        logging.info(f"💾 Результаты поиска для '{query}' закэшированы")
    except Exception as e:
        logging.error(f"❌ Ошибка кэширования результатов поиска: {e}")

# Функция preload_track_metadata удалена - больше не нужна

async def init_http_session():
    """Инициализирует HTTP сессию для оптимизации запросов"""
    global session_pool
    try:
        if session_pool is None:
            ...
            logging.info("🌐 HTTP сессия инициализирована для оптимизации")
    except Exception as e:
        logging.error(f"❌ Ошибка инициализации HTTP сессии: {e}")
...
async def cleanup_http_session():
    ...
    try:
        ...
        logging.error(f"❌ Ошибка закрытия HTTP сессии: {e}")
...
def _ydl_download_blocking(url, outtmpl, cookiefile, is_premium=False):
    ...
    try:
        ...
        # Используем оптимизированные настройки для качественной загрузки
        ydl_opts = QUALITY_DOWNLOAD_OPTS.copy()
...
async def download_track_from_url(user_id, url):
    ...
    try:
        ...
        try:
            # Используем оптимизированные настройки для быстрого извлечения метаданных
            ...
            try:
                ...
                def search_block(q):
                    try:
                        ...
                        # Используем оптимизированные настройки для быстрого поиска
                        ydl_opts = FAST_DOWNLOAD_OPTS.copy()
...
async def monitor_performance():
    ...
    while True:
        try:
            ...
            # Статистика кэшей
            track_cache_size = len(track_metadata_cache)
            search_cache_size = len(search_cache)
            
            # Статистика очередей
            regular_queue_size = len(REGULAR_QUEUE)
            premium_queue_size = PREMIUM_QUEUE.qsize() if not PREMIUM_QUEUE.empty() else 0
            
            logging.info(f"📊 Производительность: "
                        f"Кэш треков: {track_cache_size}, "
                        f"Кэш поиска: {search_cache_size}, "
                        f"Очередь обычная: {regular_queue_size}, "
                        f"Очередь премиум: {premium_queue_size}")
            
            # Автоматическая очистка при превышении лимитов
            if track_cache_size > TRACK_CACHE_MAX_SIZE * 0.8:
                cleanup_track_cache()
                logging.info("🧹 Автоматическая очистка кэша треков")
            
            if search_cache_size > SEARCH_CACHE_MAX_SIZE * 0.8:
                cleanup_search_cache()
                logging.info("🧹 Автоматическая очистка кэша поиска")
...
async def main():
    ...
    try:
        ...
        # Инициализируем HTTP сессию для оптимизации
        try:
            await init_http_session()
            logging.info("✅ HTTP сессия инициализирована")
        except Exception as e:
            logging.error(f"❌ Ошибка инициализации HTTP сессии: {e}")
    ...
    try:
        ...
        try:
            async with download_semaphore:
                ...
                # Используем оптимизированные настройки для быстрой загрузки
