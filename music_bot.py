...
async def add_track_instant(user_id: str, url: str):
    ...
    try:
        ...
        # Создаем задачу для фоновой обработки (полная загрузка метаданных)
        task_info = {
            'user_id': user_id,
            'url': url,
            'timestamp': time.time()
    ...
    try:
        ...
        if REGULAR_QUEUE:
            try:
                ...
                # Трек уже сохранен, теперь обновляем метаданные в фоне
                try:
                    # Загружаем полные метаданные
                    full_metadata = await download_track_from_url(user_id, url)
                    if full_metadata:
                        logging.info(f"✅ Метаданные обновлены для пользователя {user_id}")
                    else:
                        logging.warning(f"⚠️ Не удалось обновить метаданные для пользователя {user_id}")
                except Exception as metadata_error:
                    logging.error(f"❌ Ошибка обновления метаданных для пользователя {user_id}: {metadata_error}")
